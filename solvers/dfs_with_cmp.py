...
def dfs_with_cmp(initial_situation: Situation) -> Optional[tuple[List[int], Statistic]]:
    ...
    1. Инициализируем стек, где каждый элемент представляет текущую ситуация и путь,
       приведший к ней.
    2. Проверяем, достигнута ли целевая ситуация — если да, возвращаем путь.
    3. Добавляем текущую ситуацию в множество посещённых.
    4. Генерируем возможные действия (0-3), сортируя их по эвристике, которая приближает
       робота к цели.
    5. Для каждого действия:
       - Если следующая ситуация валидна и ещё не посещена, добавляем её в стек с обновлённым путём.
